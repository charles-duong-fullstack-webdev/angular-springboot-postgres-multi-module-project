...
as
  select a.aircraft_id
     , a.aircraft_type_id
     , a.short_name
     , t.short_name || ' ' || a.short_name as name
     , a.long_name
...
as
  select pc.pilot_id
     ...
     , pt.last_name || ', ' || pt.first_name as pilot_name
     , pt.middle_initial
     ...
     , pt.last_name || ', ' || pt.first_name || ': ' || at.short_name || ': ' || pc.certification_number as name
...
as
  select f.flight_id
     ...
     , at.short_name || ' ' || a.short_name as aircraft_name
...
as
  select m.flight_id
     ...
     , p.first_name || ' ' || p.last_name  || ' is a Member of the Crew for Flight: ' || f.long_name as name
